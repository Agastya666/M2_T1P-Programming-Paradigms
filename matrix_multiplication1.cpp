...
int main()
{
    ...
    clock_t start_time = clock(); // get the current time before matrix multiplication
    multiply_matrices(A, B, C); // multiply matrices A and B and store the result in C
    double time_taken = double(clock() - start_time) / double(CLOCKS_PER_SEC); // calculate the execution time
